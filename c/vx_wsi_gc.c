...
EXPORT void vx_pump_zombie_gc(void) {
    for (int wid = 0; wid < MAX_WINDOWS; wid++) {
        ...
        if (status == 2) {
            ...
            // [THE EASY WAY]
            // CMD 4 already called vkQueueWaitIdle. The GPU is flushed.
            // No Proxy Wait required. Smash the memory instantly.
            for (int i = 0; i < 10; i++) {
            ...
            printf("[C-CORE] Tenant %d: Swapchain safely garbage collected instantly.\n", wid);
        }
    }
}
